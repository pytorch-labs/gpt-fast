...
def replace_linear_weight_only_int8_per_channel(module):
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            ...
            setattr(module, name, Float8ConditionalFeedForward(child.num_experts, child.intermediate_size, child.dim))
        else:
        ...
        for fqn, mod in self.mod.named_modules():
            if isinstance(mod, torch.nn.Linear):
                ...
                for name in ["w1", "w2", "w3"]:
                    fp8_weight = getattr(mod, name).to(torch.float8_e4m3fn)
                    cur_state_dict[f"{fqn}.{name}"] = fp8_weight
...
class Float8ConditionalFeedForward(nn.Module):
    def __init__(self, num_experts, intermediate_size, dim):
        super().__init__()
        self.w1 = nn.Parameter(torch.empty(num_experts, intermediate_size, dim, dtype=torch.float8_e4m3fn))
        self.w2 = nn.Parameter(torch.empty(num_experts, intermediate_size, dim, dtype=torch.float8_e4m3fn))
        self.w3 = nn.Parameter(torch.empty(num_experts, intermediate_size, dim, dtype=torch.float8_e4m3fn))

    def forward(self, x, expert_indices):
        w1_weights = self.w1.to(x.dtype)[expert_indices].transpose(-1, -2) # [T, A, D, D]
        w3_weights = self.w3.to(x.dtype)[expert_indices].transpose(-1, -2) # [T, A, D, D]
        w2_weights = self.w2.to(x.dtype)[expert_indices]  # [T, A, D, D]
...
def quantize(
    ...
    if mode == 'fp8':
        print("Quantizing model weights for fp8 weight-only symmetric per-channel quantization")
